...
import openai
from .utils import calculate_token_length, getTokenLimit
...
class RetrieveThenReadApproach(Approach):
    ...
    system_chat_template = "You are an auditor reviewing a SOC 1 report and need to answer questions based on the text provided in key Source:."
    ...
    def remove_similar_results(self, results, content=None):
        max_retries = 5
        message_builder = MessageBuilder(
            self.system_chat_template, self.chatgpt_model, False
        )
    ...
    def openai_only_prompt_read(self, results):
        max_retries = 5
        message_builder = MessageBuilder(
            self.system_chat_template, self.chatgpt_model, False
        )
    ...
    def openai_only_prompt(self, results, content=None):
        max_retries = 5
        message_builder = MessageBuilder(
            self.system_chat_template, self.chatgpt_model, False
        )
    ...
    def remove_duplicates(self, results, content=None):
        max_retries = 5
        message_builder = MessageBuilder(
            self.system_chat_template, self.chatgpt_model, False
        )
        ...
        for i in range(self.openai_retries):
            try:
                ...
                chat_completion = response.model_dump_json()
    ...
    def run(
        ...
        if has_vector:
            vectors.append(self.compute_text_embedding(search_client_message))
        ...
        if fetchExtra:
            ...
            for result in results:
                ...
                try:
                    additional_results = self.search(
                        top,
                        query_text,
                        filter,
                        vectors,
        ...
        if (
            "Complementary User Entity controls" in search_client_message
            or "Complementary Subservice Organization Controls" in search_client_message
        ):
            new_results = []
            # Process the search results
            for result in results:
        ...
        while startindex < len(results):
            batchsize = (
                self.openai_batch_size
                if startindex + self.openai_batch_size < len(results)
                else len(results) - startindex
            )
            result = results[startindex : startindex + batchsize]
            sources_content = self.get_sources_content(
                result, use_semantic_captions, use_image_citation=False
            )
            ...
            token_length = calculate_token_length(
                self.openai_chatgpt_model, " ".join(map(str, message_builder.messages))
            )
            while token_length > tokenlimit:
                ...
                message_builder.insert_message("user", user_content)
                token_length = calculate_token_length(
                    self.openai_chatgpt_model,
                    " ".join(map(str, message_builder.messages)),
                )
            # user_content = [q]
            ...
            for i in range(self.openai_retries):
                try:
                    ...
                    chat_completion = response.model_dump()
