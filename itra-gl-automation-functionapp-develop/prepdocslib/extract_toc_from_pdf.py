...
class IndexPageType(Enum):
        ...
        WITHPLAINTEXT = 4,
        NONE = 5
roman_chars = ['I ', 'II ', 'III ', 'IV ', 'V ','= ','I:', 'II:', 'III:', 'IV:', 'V:','=:']
...
class PDFTableOfContentsExtractor:
    ...
    def handleIndexPageType(self,indexPageType, values):
        ...
        if len(titles) > 0:
            ...
            titles = [value for value in titles if "table of contents" not in value.lower() and "contents" not in value.lower() and  "table of content" not in value.lower() and "content" not in value.lower()]
            titles = [value.rstrip() if value[-1].isspace() and value[-2].isdigit() else value for value in titles]
            titles = [value for value in titles if not value.isdigit()]
            titles = [re.sub(r'\d+$', '', value) for value in titles if value]
            titles = [value.strip() for value in titles]
            titles = [value[:-1] if value[-1] == '.' else value for value in titles if value]
            return titles
        return []
    def find_page_number(self,pages:List[DocumentPage]):
        pass
    def extract_toc_fromparagraph(self,paragraphs:List[DocumentParagraph]) -> List[str]:
        paragraph = [paragraph for paragraph in paragraphs if any(keyword.lower() in paragraph.content.lower() for keyword in ["table of contents", "contents"])]
        if paragraph and len(paragraph) > 0:
            bounding_region = paragraph[0].bounding_regions[0]
            pageNo = bounding_region.page_number
            sectiontitles = [paragraph for paragraph in paragraphs if paragraph.bounding_regions[0].page_number == pageNo]
